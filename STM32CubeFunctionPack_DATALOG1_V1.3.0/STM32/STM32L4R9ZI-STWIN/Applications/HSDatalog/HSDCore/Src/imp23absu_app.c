...
static void IMP23ABSU_Thread(void const *argument)
{
  ...
  for (;;)
  {
    if (IMP23ABSU_Sensor_State == SM_SENSOR_STATE_INITIALIZING)
    {
      ...
      HAL_DFSDM_FilterRegularStart_DMA(&hdfsdm1_filter1, (int32_t*)amic_mem, ((uint32_t)IMP23ABSU_Init_Param.ODR[0] / 1000) * IMP23ABSU_MS * 2);
    ...
    else if(IMP23ABSU_Sensor_State == SM_SENSOR_STATE_RUNNING)
    {
      ...
      if(IMP23ABSU_Sensor_State == SM_SENSOR_STATE_RUNNING) /* Change of state can happen while task blocked */
      {
        ...
        IMP23ABSU_Data_Ready(0, (uint8_t *)data_ptr, ((uint32_t)IMP23ABSU_Init_Param.ODR[0] / 1000) * IMP23ABSU_MS * 2, TimeStamp_imp23absu);
      }
    }
...
void DFSDM_Filter_1_Complete_Callback(DFSDM_Filter_HandleTypeDef *hdfsdm_filter)
{
  ...
  {
    osMessagePut(amicDreadyQueue_id, (uint32_t)(&amic_mem[(((uint32_t)IMP23ABSU_Init_Param.ODR[0]/1000) * IMP23ABSU_MS)]), osWaitForever);
  }
}
