...
static void IMP34DT05_Thread(void const *argument)
{
  ...
  for (;;)
  {
    if (IMP34DT05_Sensor_State == SM_SENSOR_STATE_INITIALIZING)
    {
      ...
      HAL_DFSDM_FilterRegularStart_DMA(&hdfsdm1_filter0, (int32_t*)dmic_mem, ((uint32_t)IMP34DT05_Init_Param.ODR[0] / 1000) * IMP34DT05_MS * 2);
    ...
    else if(IMP34DT05_Sensor_State == SM_SENSOR_STATE_RUNNING)
    {
      ...
      if(IMP34DT05_Sensor_State == SM_SENSOR_STATE_RUNNING) /* Change of state can happen while task blocked */
      {
        ...
        IMP34DT05_Data_Ready(0, (uint8_t *)data_ptr, ((uint32_t)IMP34DT05_Init_Param.ODR[0] / 1000) * IMP34DT05_MS * 2, TimeStamp_imp34dt05a);
      }
    }
...
void DFSDM_Filter_0_Complete_Callback(DFSDM_Filter_HandleTypeDef *hdfsdm_filter)
{
  ...
  osMessagePut(dmicDreadyQueue_id, (uint32_t)(&dmic_mem[(((uint32_t)IMP34DT05_Init_Param.ODR[0]/1000) * IMP34DT05_MS)]), osWaitForever);
}
