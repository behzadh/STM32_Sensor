...
{ 
  ...
  if(ISM330DHCX_Init_Param.subSensorActive[0] && ISM330DHCX_Init_Param.subSensorActive[1])      /* Read both ACC and GYRO */
  {
...
{  
  COM_SubSensorStatus_t *pSubSensorStatus = COM_GetSubSensorStatus(ISM330DHCX_Get_Id(), 2);  
  if (pSubSensorStatus->ucfLoaded == 0 && pSubSensorStatus->isActive == 1)      /* If you enter here, MLC isActive is true but no UCF are avalable */
...
static void ISM330DHCX_Sensor_Init(void)
{
  ...
  if(ISM330DHCX_Init_Param.subSensorActive[0] && ISM330DHCX_Init_Param.subSensorActive[1])      /* Both subSensor is active */
  {
